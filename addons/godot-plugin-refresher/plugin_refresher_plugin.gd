...
func _reload_plugins_list():
	...
	if dir:
		...
		while file != "":
			...
			if dir.current_is_dir() and file != refresher_dir:
				var display_name = file
				var plugin_config_path = addon_dir.path_join("plugin.cfg")
				# Check if file exists relative to the opened directory or absolute?
				# DirAccess.open opens a dir. 'file_exists' checks inside it (if passed filename) or absolute path?
				# Godot 4 dir.file_exists checks relative if not absolute.
...
func get_config_path():
	var home = "res://".path_join(PLUGIN_CONFIG_DIR)
